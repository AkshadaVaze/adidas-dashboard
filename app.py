...
if selected_input == 'Description and data':
    ...
    with col4:
        selected_input=st.selectbox("Select the Product:",df['Product'].unique())
        filtered_df=df[df['Product']==selected_input]
        fig = px.bar(filtered_df, x = "Retailer", y = "TotalSales", labels={"TotalSales" : "Total Sales {$}"},
    ...
    with col7: 
        selected_input=st.selectbox("Select a State:",df['State'].unique())
        filtered_df=df[df['State']==selected_input]
        st.subheader("Pie chart of Sales Method")
        fig2,ax1=plt.subplots()
        ax1.pie(x=filtered_df['SalesMethod'].value_counts().values,labels=filtered_df['SalesMethod'].value_counts().index,autopct="%0.2f%%" )
        st.pyplot(fig=fig2)
    with col8:
        selected_input=st.selectbox("Select the Retailer:",df['Retailer'].unique())
        filtered_df=df[df['Retailer']==selected_input]
        st.subheader("Count plot of Regions")
        fig3=px.bar(data_frame=filtered_df,x=filtered_df['Region'].value_counts().index,y=filtered_df['Region'].value_counts().values)
